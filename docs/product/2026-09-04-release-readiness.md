...
Initial audited checkout: `codex/adaptive-media-shadow`, HEAD `57c5ba7`. Local Blob catalogue implementation subsequently reached `0a85f2c`, followed by Task 7 cleanup at `daa44ec`. Task 8 freshly verified that implementation and recorded the local/browser and remote-gate boundaries below. Separate playback style/test changes and current player runtime tests remain uncommitted; the pre-existing `.DS_Store` modification is preserved. The research-only review in the main checkout does not cover these shadow-worktree changes.
...
| Browser, accessibility, device/fidelity checks | Fresh Task 8 local injected-HTTP run in in-app Chromium 152.0.7977.64 | Mixed Music order/navigation, 390px/938px eligibility, fullscreen control behavior, periodic refresh, failure retention, current/inactive removal, and safe title rendering were observed with synthetic loopback media. Native focus/visibility acceptance, full fullscreen visual acceptance, Safari, actual-phone listening, and remote comparison remain open. |
...
The [Task 8 verification record](2026-09-04-vercel-blob-folder-catalogue-verification.md) supersedes earlier evidence for the current local checkpoint: all 353 tests and 70 focused tests passed with zero failures/skips; both build modes and diff hygiene passed. The scoped source scan found no Supabase/project/private-key reference and only the intentional fake-token assignment in an endpoint redaction test. The local endpoint returned GET 200, POST 405, and generic no-store 503 as expected; synthetic MP4 range delivery returned 206. Browser checks used an explicit test-only HTTPS-candidate-to-loopback mapping before native metadata probing, and all non-local requests were blocked. These results do not prove Vercel Function packaging, Blob/CDN behavior, real media delivery, or release readiness.

The exact proposed Preview target, four initial public object names, one move destination, conditional SDK operations, observation steps, and rollback are documented there and remain unexecuted. The existing project/store/Preview linkage must be revalidated after approval; the Vercel CLI is currently absent. Same-path replacement reuses the public media URL, so actual replacement bytes under ordinary browser/CDN caching must be tested rather than inferred from a changed ETag. Local Node was 26.4.0 while the linked project's configuration specifies Node 24.x; remote-runtime acceptance remains open.
...
2. Complete the remaining native focus/visibility, fullscreen visual, Safari, and actual-phone acceptance checks. The successful local in-app run does not certify these remaining browser/device gates.
